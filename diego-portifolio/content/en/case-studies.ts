  ...
  items: [
    {
      ...
      decisions: [
        ...
        },
        {
          ...
          body: "an order's status can only change through a dedicated method, direct writes throw an exception, and invariants such as a single open tab per table are guaranteed in the model itself. The rule does not depend on who calls it.",
        },
        {
    ...
    },
    {
      ...
      built:
        "A dashboard builder with a catalog of more than 30 widget models, per-widget configuration, real-time updates over MQTT, and streaming to the browser — delivered on web, in a native mobile app, and in the API that powers both. Also an interactive 3D industrial plant, with a machine catalog, per-client saved layouts with concurrency control, links to real devices, later ported to mobile — and the pipeline that generates these 3D models with generative AI (Google Gemini), running on an asynchronous Python worker.",
      decisions: [
        {
        ...
        {
          lead: "AI-generated code treated as untrusted",
          body: "the 3D model generated by Gemini runs in an asynchronous Python worker, is validated in a sandbox, and only enters the catalog after passing an automated quality gate.",
        },
      ],
      result:
